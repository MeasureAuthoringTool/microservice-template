...
public class ActionLogServiceTest {
  ...
  void testLogActionReturnsTrue() {
    when(actionLogRepository.pushEvent(anyString(), any(Action.class))).thenReturn(true);
    boolean output = actionLogService.logAction("TARGET_ID", ActionType.CREATED, "testUser");
    assertThat(output, is(true));
    verify(actionLogRepository, times(1))
        .pushEvent(targetIdArgumentCaptor.capture(), actionArgumentCaptor.capture());
  ...
  void testLogActionReturnsFalse() {
    when(actionLogRepository.pushEvent(anyString(), any(Action.class))).thenReturn(false);
    boolean output = actionLogService.logAction("TARGET_ID", ActionType.DELETED, "testUser");
